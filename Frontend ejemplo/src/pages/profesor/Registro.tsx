...
import { validarEmailUDD } from '../../utils/helpers';
...
export default function ProfesorRegistro() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar email UDD
    if (!validarEmailUDD(formData.email)) {
      toast.error('Error', {
        description: 'Debe ser un correo @udd.cl'
      });
      return;
    }
    ...
    if (!formData.nombre || !formData.apellidos || !formData.codigo) {
      toast.error('Error', {
    ...
    try {
      await registro({
        ...
        correo_udd: formData.email,
        codigo_profesor: formData.codigo,
        contrasena: formData.password,
        id_tipoUsuario: 1 // Profesor
      });
